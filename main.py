...
def remove_dummy_multi_method(cl, thread_id, dummy_user_id):
    ...
    # Method 1: direct_remove_user (if available)
    try:
    ...
    # Method 3: direct_messages.remove_user_from_group
    try:
...
def create_gcs_stream(session_id, group_count, usernames, remove_username, custom_message, delay_seconds):
    ...
    for u in usernames:
        try:
            ...
            yield sse_event("error", f"❌ Error resolving {u}: {e}")
            return
    ...
    if remove_username:
        try:
            ...
            yield sse_event("warn", f"⚠️ Error resolving {remove_username}: {e}")
    ...
    for i in range(1, group_count + 1):
        ...
        try:
            ...
            if send_result is None:
                yield sse_event("error", f"❌ GC {i} send failed (no response)")
                continue
        ...
        if thread_id:
            ...
            if dummy_user_id:
                ...
                yield sse_event("info", f"ℹ️ No dummy user to remove")
        else:
